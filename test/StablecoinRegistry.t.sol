...
contract StablecoinRegistryTest is Test {
    ...
    function setUp() public {
        ...
        // Register stablecoins (decimals auto-detected from contract)
        registry.registerStablecoin(address(usdc), "USDC", "US", 1e8);          // 1:1 USD
        registry.registerStablecoin(address(usdt), "USDT", "US", 1e8);          // 1:1 USD  
        registry.registerStablecoin(address(idrx), "IDRX", "ID", 16000e8);      // 16000:1 USD
        registry.registerStablecoin(address(jpyc), "JPYC", "JP", 150e8);        // 150:1 USD
        registry.registerStablecoin(address(euroc), "EUROC", "EU", 0.92e8);     // 0.92:1 USD
    ...
    function testRegisterStablecoin() public {
        ...
        assertEq(info.decimals, 8); // Auto-detected from contract
    ...
    function testUpdateRateEnforcesChangeLimit() public {
        ...
        uint256 tooHighRate = 2e8; // 100% increase, exceeds 50% limit
    ...
    function testSetEthUsdRateEnforcesBounds() public {
        ...
        registry.setEthUsdRate(500e8); // Below MIN_ETH_USD_RATE (1000e8)
        
        vm.prank(owner);
        vm.expectRevert("Registry: rate too high");
        registry.setEthUsdRate(200000e8); // Above MAX_ETH_USD_RATE (100000e8)
    }
    ...
    function testConvertUSDCToUSDT() public {
        ...
        // Should be approximately equal (both are 1:1 USD)
        assertApproxEqRel(usdtResult, 100 * 10**6, 0.01e18); // 1% tolerance
    }

    function testConvertUSDCToIDRX() public {
        uint256 usdcAmount = 1 * 10**6; // 1 USDC = $1
        uint256 idrxResult = registry.convert(address(usdc), address(idrx), usdcAmount);
        
        // 1 USD = 16,000 IDRX
        // IDRX has 2 decimals, so 16,000 IDRX = 16000 * 10^2
        assertEq(idrxResult, 16000 * 10**2, "1 USDC should equal 16000 IDRX");
    }

    function testConvertIDRXToUSDC() public {
        uint256 idrxAmount = 16000 * 10**2; // 16,000 IDRX = $1
    ...
    function testConvertWithDifferentDecimals() public {
        // JPYC has 18 decimals, USDC has 6
        uint256 jpycAmount = 150 * 10**18; // 150 JPYC = $1
    ...
    function testEthToToken() public {
        ...
        // 1 ETH * $3000 (default rate) = $3000 = 3000 USDC
    ...
    function testEthToTokenDifferentRates() public {
        ...
        // $300 = 4,800,000 IDRX (300 * 16000 = 4,800,000)
    ...
    function testTokenToEth() public {
        uint256 usdcAmount = 3000 * 10**6; // $3000
    ...
    function testGetAllStablecoins() public {
        ...
        assertEq(tokens.length, 5); // USDC, USDT, IDRX, JPYC, EUROC
    ...
    function testGetActiveStablecoins() public {
        ...
        assertEq(active.length, 4); // USDT, IDRX, JPYC, EUROC
    }
    ...
    function testGetStablecoinsByRegion() public {
        ...
        assertEq(usTokens.length, 2); // USDC, USDT
        
        address[] memory idTokens = registry.getStablecoinsByRegion("ID");
        assertEq(idTokens.length, 1); // IDRX
        
        address[] memory jpTokens = registry.getStablecoinsByRegion("JP");
        assertEq(jpTokens.length, 1); // JPYC
    }

    function testGetRateBounds() public {
        (uint256 minRate, uint256 maxRate, uint256 maxChange, uint256 minEth, uint256 maxEth) 
            = registry.getRateBounds();
        
        assertEq(minRate, 1e4);        // MIN_RATE_TO_USD
        assertEq(maxRate, 1e16);       // MAX_RATE_TO_USD
        assertEq(maxChange, 5000);     // MAX_RATE_CHANGE_BPS (50%)
        assertEq(minEth, 1000e8);      // MIN_ETH_USD_RATE = 1000e8
        assertEq(maxEth, 100000e8);    // MAX_ETH_USD_RATE = 100000e8
    }
